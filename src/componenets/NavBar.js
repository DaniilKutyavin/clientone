...
import { CONTACTS_ROUTER, DELIVERY_ROUTER, NEWS_ROUTER, SHOP_ROUTER, LOGIN_ROUTER, REGISTRATION_ROUTER, PRODUCT_ROUTER, LS_ROUTER } from "../utils/consts";
...
const NavBar = () => {
    const { user } = useContext(Context); // Получаем user из контекста
    ...
    const handleUserIconClick = () => {
        if (user.isAuth) { // Проверяем, авторизован ли пользователь
            navigate(LS_ROUTER); // Перенаправляем на /product
        } else {
